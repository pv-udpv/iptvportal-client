...
def demo_config_files():
    ...
    for i, file_path in enumerate(files, 1):
        ...
        path = Path(file_path)
        display_path = path.relative_to(Path.cwd()) if path.is_relative_to(Path.cwd()) else path
...
def demo_use_cases():
    ...
    print("    This uses incremental sync for frequently updated data")

    print("\n  For package table:")
    print(f"    strategy = {conf.sync.package.strategy}")
    print(f"    ttl = {conf.sync.package.ttl}s")
    print("    This uses on-demand sync for rarely changing data")
