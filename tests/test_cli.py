...
def test_query_select_help():
    """Test query select command help."""
    result = runner.invoke(app, ["query", "select", "--help"])
    assert result.exit_code == 0
    assert "SELECT" in result.stdout

def test_query_insert_help():
    """Test query insert command help."""
    result = runner.invoke(app, ["query", "insert", "--help"])
    assert result.exit_code == 0
    assert "INSERT" in result.stdout

def test_query_update_help():
    """Test query update command help."""
    result = runner.invoke(app, ["query", "update", "--help"])
    assert result.exit_code == 0
    assert "UPDATE" in result.stdout

def test_query_delete_help():
    """Test query delete command help."""
    result = runner.invoke(app, ["query", "delete", "--help"])
    assert result.exit_code == 0
...
def test_transpile_with_where():
    ...
    result = runner.invoke(app, ["transpile", "SELECT id, username FROM subscriber WHERE disabled = false"])
    assert result.exit_code == 0
    assert '"from": "subscriber"' in result.stdout
...
def test_query_select_dry_run():
    """Test query select with dry-run mode."""
...
def test_query_select_from_sql_dry_run():
    """Test query select with --from-sql and dry-run."""
    result = runner.invoke(app, [
        "query", "select",
...
def test_query_insert_dry_run():
    """Test query insert with dry-run mode."""
    result = runner.invoke(app, [
        "query", "insert",
    ...
    assert result.exit_code == 0
    assert "DRY RUN MODE" in result.stdout
    assert '"into": "package"' in result.stdout

def test_query_update_dry_run():
    """Test query update with dry-run mode."""
    result = runner.invoke(app, [
        "query", "update",
...
def test_query_delete_dry_run():
    """Test query delete with dry-run mode."""
    result = runner.invoke(app, [
        "query", "delete",
...
def test_query_select_missing_from():
    """Test that query select requires --from when not using --from-sql."""
    result = runner.invoke(app, [
        "query", "select",
        "--data", "id,username"
    ])
...
def test_query_insert_missing_params():
    """Test that query insert requires parameters when not using --from-sql."""
    result = runner.invoke(app, [
        "query", "insert",
        "--into", "package"
    ])
...
def test_query_update_missing_params():
    """Test that query update requires parameters when not using --from-sql."""
    result = runner.invoke(app, [
        "query", "update",
        "--table", "subscriber"
    ])
...
def test_query_delete_missing_from():
    """Test that query delete requires --from when not using --from-sql."""
    result = runner.invoke(app, [
        "query", "delete",
        "--where", '{"eq": ["id", 123]}'
    ])
