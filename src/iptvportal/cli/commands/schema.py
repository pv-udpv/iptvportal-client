...
from iptvportal.cli.utils import load_config, display_json
...
def from_sql_command(
    ...
    fields: Optional[str] = typer.Option(None, "--fields", help="Manual field mappings (e.g., '1:name,2:email,3:url')"),
...
def validate_command(
    ...
    try:
        ...
        for table_name in tables:
            ...
            for pos, field in schema.fields.items():
...
def introspect_command(
    ...
    try:
        ...
        console.print(f"[green]✓ Introspection complete[/green]\n")
...
def clear_command(
    table_name: Optional[str] = typer.Argument(None, help="Table name to clear (omit to clear all)"),
    config_file: Optional[str] = typer.Option(None, "--config", "-c", help="Config file path"),
